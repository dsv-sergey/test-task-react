...
export const MainPage = () => {
    ...
    const [page, setPage] = useState(
        uuiRouter.getCurrentLink().query['page'] || 1,
    );
    const [search, setSearch] = useState<string>();
    const { data, isError, error, isLoading, isFetching, totalPages } =
    ...
    const isNoData =
        !isLoading && !isFetching && !isError && isEmpty(data?.results);
    ...
    return (
        <div className={css.wrapper}>
            ...
            <main className={css.main}>
                {isError && (
                    <Text cx={css.alignMessage} font="Prompt" fontSize="24">
                        {error.message}
                    </Text>
                )}
                {isNoData && (
                ...
                {!isError && !isNoData && (
                    <FlexBox enableLoading={isLoading || isFetching}>
                        {!isEmpty(data?.results) && (
                            <PersonList persons={data?.results} />
                        )}
                    </FlexBox>
                )}
            </main>
