...
import { SuccessNotification, WarningNotification, useForm, Text, FlexRow, Button, LinkButton, FlexSpacer, Blocker, ScrollBars } from "@epam/uui";
...
export const PersonPage = () => {
    ...
    const { lens, save, replaceValue, isChanged } = useForm<IPerson>({
        ...
        onSave: state => {
            ...
            if(pageData) {
                queryClient.setQueryData(["PERSONS_PAGE", page], (input: IPersonsRequest) => {
                    return {
                        ...input,
                        results: [
                            ...input.results.map(value => {
    ...
    const getTitle = () => (
        <>
            <LinkButton cx={ css.backButton } icon={ BackIcon } onClick={ goBack } color="white" size="48" caption='Back' />
            { query.data?.name }
        </>
    );
